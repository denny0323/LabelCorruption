...
def trainBoot(model, device, train_loader, train_loader_track, test_loader, learning_rate, weight_decay, epochs, restart, mixture, max_iters, 
NOISE_LEVEL,  batch_size, test_batch_size, log_interval, milestones, reg_term=0, TYPE='Hard', train_type='Boot'):
    
    #bootstrap_ep_std = milestones[0] + 5 + 1 # the +1 is because the conditions are defined as ">" or "<" not ">="
    bootstrap_ep_std = milestones[0]
    ...
    for r in range(restart+1):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if epoch <= bootstrap_ep_std:
                ...
                if mixture=='BMM':
                    if TYPE == "Hard":
                        print("\n\t##### Doing HARD BETA bootstrapping from the epoch {}__{} #####".format(bootstrap_ep_std, mixture))
                        loss_per_epoch, acc_train_per_epoch_i = train_DYN_Bootstrapping(model, device, train_loader, optimizer, epoch, epochs,\
                                                                                            TYPE, mixture, mixture_model, bmm_model_maxLoss, bmm_model_minLoss, batch_size, log_interval, reg_term)
                    elif TYPE == "Soft":
                        print("\n\t##### Doing SOFT BETA bootstrapping from the epoch {}__{} #####".format(bootstrap_ep_std, mixture))
                        loss_per_epoch, acc_train_per_epoch_i = train_DYN_Bootstrapping(model, device, train_loader, optimizer, epoch, epochs,\
                                                                                            TYPE, mixture, mixture_model, bmm_model_maxLoss, bmm_model_minLoss, batch_size, log_interval, reg_term)
            ...
            epoch_losses_train, epoch_probs_train, argmaxXentropy_train, mixture_model, bmm_model_maxLoss, bmm_model_minLoss = \
                track_training_loss(model, device, train_loader_track, epoch, mixture, max_iters, mixture_model, bmm_model_maxLoss, bmm_model_minLoss)
            ...
            if epoch == 1:
                ...
                snapBest = 'best_epoch_%d_valLoss_%.5f_valAcc_%.5f_noise_%.2f_bestAccVal_%.5f' % (
                    epoch, loss_per_epoch_test[-1], acc_val_per_epoch_i[-1], NOISE_LEVEL, best_acc_val)
                directory = './{}/{}'.format(train_loader.dataset.noise_mode, TYPE) # noise type / train_type / pattern
                os.makedirs(directory, exist_ok=True)
                torch.save(model.state_dict(), os.path.join(directory, snapBest + '.pth'))
                print('└ save the best model in {}/{}\n'.format(directory, snapBest+'.pth')) 
                ...
                #else:
                #    if epoch == 5:
                #        snapBest = '%d_%s_best_epoch_%d_valLoss_%.5f_valAcc_%.5f_noise_%.2f_bestAccVal_%.5f' % (r+1, mixture, epoch, loss_per_epoch_test[-1], acc_val_per_epoch_i[-1], NOISE_LEVEL, best_acc_val)
                #        torch.save(model.state_dict(), os.path.join(directory, snapBest + '.pth'))
