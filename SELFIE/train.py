...
def train_CrossEntropy(model, device, train_loader, CLASS, optimizer, epoch, epochs, batch_size, log_interval):
    model.train()
    loss_per_batch = []
    acc_train_per_batch = []
    correct = 0
    ...
    for batch_idx, (Id, data, target) in enumerate(train_loader):
        data, target = data.to(device), target[:, CLASS].to(device)
        optimizer.zero_grad()
...
def trainSELFIE(model, device, train_loader, test_loader, learning_rate, weight_decay, epochs, restart, NOISE_LEVEL,  batch_size, test_batch_size, log_interval, milestones, history_length, reg_term=0, train_type='SELFIE'):
    ...
    for r in range(restart+1):
        ...
        historybank = History_Bank(num_data, history_length=history_length, num_of_classes=7)
        cont = 0
        print('\n########################### {} run ###########################'.format(r+1))
